<?php
    ...
    $album = function($parameters)
    {
        ...
        $sql = "SELECT DISTINCT Album.Code_Album as Code_Album, Disque.Code_Disque as Code_Disque, Morceau.Code_Morceau as Code_Morceau, Enregistrement.Titre as Titre, Enregistrement.Durée as Duree, Enregistrement.Prix as Prix, Enregistrement.Extrait as Extrait FROM Album ";
        ...
        $results = $_system_registry->getModel()->query("SELECT DISTINCT Album.Code_Album as Code_Album, Album.Titre_Album as Titre_Album, Album.Année_Album as Annee_Album, Album.Pochette as Pochette, Album.ASIN as ASIN WHERE Album.Code_Album = '".$parameters["code"]."'")->fetchall();
        // Récupère les informations relatives à l'album
        $parameters["Titre_Album"] = $results[utf8_decode("Titre_Album")];
        $parameters["Annee_Album"] = $results[utf8_decode("Annee_Album")];
